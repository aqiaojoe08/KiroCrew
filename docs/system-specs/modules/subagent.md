...
A synchronous non-batch facade rejection still returns the counted error in the
HTTP response, but its terminal transaction inserts a pending event and attempts
the ordinary parent completion callback immediately. If that handoff fails, the
event remains restart-drainable instead of being marked delivered solely because
the HTTP caller saw the response. Batch rejections use the same pending delivery
rule, preserve `batch_id` and `batch_total`, and settle their wave through the
