...
def _create_file_blocking(target: Path, content: str) -> tuple[str | None, str]:
    ...
    try:
        ...
        try:
            ...
            if leaf is not None:
                pinned_fs.unlink_verified(dir_fd, target.name, (leaf.st_dev, leaf.st_ino))
            else:
                # The fstat itself failed, so there is no identity to verify against.
                # Remove the name best-effort rather than leaving an empty O_EXCL file
                # that answers every retry with "exists" until someone deletes it by
                # hand. Losing the identity is NOT evidence of a rival: this call still
                # holds the descriptor, and fstat succeeds on an inode a rival has
                # already unlinked -- so a failure here means the filesystem is in
                # trouble, not that the name changed hands. Deleting the wrong file
                # would need that failure AND a rival swap in the same window, two
                # independent rare events, against a certain permanent conflict if the
                # name is left behind.
