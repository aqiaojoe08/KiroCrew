...
class SkillsLoader:
    ...
    def _create_skill_pinned(
        ...
        try:
            ...
            #
            # ``leaf`` is None only when the leaf open or its fstat failed, which
            # means no bytes were written.
            if leaf is not None:
                pinned_fs.unlink_verified(dir_fd, "SKILL.md", (leaf.st_dev, leaf.st_ino))
            else:
                # No identity because the fstat itself failed. Remove the name
                # best-effort: leaving it makes the directory non-empty, which makes
                # remove_dir_verified's rmdir fail, which strands the whole create
                # behind a permanent 409. Losing the identity is NOT evidence of a
                # rival -- this call still holds the descriptor, and fstat succeeds on
                # an inode a rival has already unlinked -- so it means the filesystem
                # is in trouble. Hitting the wrong file needs that failure AND a rival
                # swap in the same window.
                with suppress(OSError):
