...
def test_a_failed_identity_probe_still_leaves_no_partial_file(tmp_path, monkeypatch):
    """An ``fstat`` that fails must be rolled back, not escape leaving the O_EXCL name.

    The rollback verifies identity, so it needs one — and capturing it is a syscall
    that can fail (EIO, ESTALE on a network filesystem). Outside the guarded region
    that failure escapes with the empty ``O_EXCL`` file still on disk, and every retry
    answers ``exists`` until someone deletes it by hand.
    """
    _needs_openat()
    target = tmp_path / "steering" / "rules.md"
    real_fstat = os.fstat

    def fail_the_leaf_probe(fd):
        info = real_fstat(fd)
        if stat.S_ISREG(info.st_mode):
            raise OSError(errno.EIO, "identity probe failed")
        return info

    monkeypatch.setattr(os, "fstat", fail_the_leaf_probe)
    err, _display = _create_file_blocking(target, "BODY")
    monkeypatch.undo()

    assert err == "writefailed"
    assert not target.exists(), "the empty O_EXCL name survived a failed identity probe"
