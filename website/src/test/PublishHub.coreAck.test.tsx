...
describe('PublishHub — a core destination reaches the exposure acknowledgment', () => {
  ...
  it('tells a core destination the truth about how the exposure ends', async () => {
    ...
    // Names the ONE exit that exists in the dashboard: deleting the artifact. This
    // previously required "private or unpublish", which swapped one unavailable way out
    // for another -- `api.unpublishArtifact` and `api.updateArtifactSharing` are defined
    // but called from zero components, so neither control is reachable by a user.
